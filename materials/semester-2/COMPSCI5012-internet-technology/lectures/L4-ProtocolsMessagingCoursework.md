# L4-ProtocolsMessagingCoursework
...
Use forms and URLs in Django (own-study, 
TwD
 Chapters 7 & 8)
...
The OSI model:
7 layers for network communication
Application
...
The TCP/IP model:
4 layers for network communication
TCP/IP Model
We will discuss:
Application Layer
HTTP
...
Protocols:
IP: 
Deliver packets according to IP address
Link/Physical
...
set of conventions that determine how data is formatted, transmitted, and received over the network
Type of messages
, e.g. request/response
Message syntax
, e.g. fields in messages
Message semantics
, e.g. what fields mean
 Rules
...
A 
Request-Response 
pattern is a way to exchange messages.
...
(where the server, middleware, application logic live)
Path:
...
The 
Domain name
 (e.g. gla.ac.uk), but the internet's languages is 
IP addresses
DNS: Domain Name System
 – A DNS server will translate the domain name to an IP address
Client
Web Application
User clicks on URL 
? . User views webpage
...
Domain Name Server
2. Browser sends a DNS request to the DNS server (DNS query)
3. DNS server resolves domain name to IP address
(DNS response)

4. Browser sends an HTTP request for the webpage
...
Domain Name Server
2. Browser sends a DNS request to resolve (query) to the DNS server 
3. DNS server resolves domain to IP address
5. Web applications responds with the webpage 

Requests and Responses (1)
T
h
e following happens when the 
user agent 
(i.e. the web browser/client) is asked to 
send
 a 
message
 (i.e. when the user clicks a link): 
First, the URL is turned into an 
IP address
Request: 
ask Domain Name System (DNS) for IP
Response:
 returns the IP for the URL
E.g. 
www.gla.ac.uk
 maps to 
130.209.16.90
...
Second, a 
TCP connection
 is opened on a particular port on the node at that IP address
...
Request:
 Get the home page of the specified URL.
Response: 
Returns the HTML for the home page
For a full ports list: 
...
Further requests for Style Sheets and 
Javascript

Application-layer Protocols
Requests can be made using various protocols
http
:  This is the most common protocol and indicates a file that a web browser can format and display - an HTML file, image file, sound file, etc.
https
: Automatically performs Secure Socket Layer negotiation and always sends data in encrypted form
file
: This indicates a file which is not in a recognised web format and will be displayed as text
Other protocols: ftp, 
mailto
, news, telnet, ...
...
ftp
: File transfer protocol is used to refer to web sites from which files can be extracted and downloaded to the client machines
mailto
: If selected, a link generates a form in which an e-mail message can be constructed and sent to a designated user
news
:  The resource is a news group or article
telnet
:  generates a telnet session to a server
...
is used to deliver virtually all files and other data using 
8 bit characters
Usually, HTTP takes place through TCP/IP sockets
HTTP is used to transmit 
resources
, not just files. 
...
request message
 to an HTTP server
Request messages are typically 
GET 
or 
POST 
methods
...
response message
, usually containing the resource that was requested. 
Respones
 are typically in XHTML, XML,  but also in other formats like JSON
After delivering the response, the server 
closes the connection
 making HTTP a stateless protocol
...
Example: 
https://www.google.com/search?q=itech
...
HTTP GET (2)
User benefits: 
The data is part of the URL – the user can see them, copy them, bookmark them, resubmit the page easily
When to use GET? 
GET should be used for pages which 
don’t change anything on the server 
...
How HTTP POST works
Unlike GET, POST includes data in the 
message body
, not in the URL.
Required headers
...
yyy
”	
The structure ( paramName1 = "paramValue1"&paramName2=....) is defined by the 
x-www-form-urlencoded 
standard (Content-Type in the header)
...
Max 2048 characters, only ASCII
No restrictions
Security
Less secure – data is part of the URL
More secure
...
HEAD
 is just like GET, except it asks the server to return the response headers only
useful to check characteristics of a resource without actually downloading it
PUT
 for storing data on the server
DELETE
 for deleting a resource on the server
OPTIONS
 for finding out what the server can do - e.g. switch to secure connections
TRACE
 for debugging connections
CONNECT
 for establishing a link through a proxy
...
to
 close connection
Once the server sends the HTTP response, it closes the TCP connection
...
unique ID - the cookie 
– and an entry in the backend database 
...
Third-party cookies allow the same 
       user to be 
tracked across multiple websites
https://i.redd.it/uh6nfugw5jf51.jpg
 

HTTP 1.0 is not persistent
Under HTTP, communication is as follows:
An HTTP client (a browser) opens a connection and sends a 
request message
 to an HTTP server to fetch a webpage
The server then returns a 
response message
, usually containing the resource that was requested. 
After delivering the response, the server 
...
The browser parses the HTML content and sees that there are 10 images referenced in the page
The HTTP client (browser) opens a connection and sends a request message to fetch the 1st image
The server then returns a response message 
The server closes the connection
…..repeat connection and request/response for every image

HTTP not being persistent is a problem
Round-trip time (RTT): 
The time required for a small message (packet) to travel from the client to the server and back
Total time to load page = 
2 RTT + time to transmit page
time to 
transmit 
page
initiate TCP
...
request+response
) time for all the referenced objects (cutting response time in half)
...
Browser caching
Browser caching
 stores some data on the browser to avoid frequent transfers from the server
How does browser caching works?
Alice uses browser on device to visit an e-commerce site for the first time.
When the first HTTP request arrives at the server, 
t
he server responds with the requested resources (HTML, CSS, images, etc.) along with 
caching headers
 
Alice’s browser stores these resources in its 
local cache
 
Subsequent HTTP requests from Alice include 
conditional GET headers
 to check whether the cached resource has changed.
If the resource hasn’t changed, the server returns a 
304 Not Modified
 response, allowing the browser to load the resource from the cache without downloading it again.
When to use?
To 
...
Concerns
Staleness!
 If not used properly, caching can result in serving outdated content
...
12 guidelines relevant to Hosting and Infrastructure 
(Section 4):  
...
gRPC
 is the most popular framework (created by Google, now open-source) on top of HTTP/2
...
XML (
eXtensible
 Markup Language) is a flexible text format used for representing structured data
Features:
Human-readable
...
XML-RPC is designed to call methods
Benefits from running over HTTP
i.e. security, encryption, authentication
But critics claim it does not add much over XML
It still requires an application level data model 
...
rpc+xml
;
charset=utf-8Content-Length: 199 
<?xml version="1.0"?>
	<
methodCall
>
...
methodName
>
examples.getStockQuote
</
methodName
>
		<params>
			<param> <value><string>IBM</string></value> </param> 
		</params>
	</
methodCall
>
...
>
		<params>
			<param> <value><double>50</double></value> </param> 
		</params>
	</
methodResponse
> 
...
Relies on XML, RPC and HTTP.
SOAP is the successor of XML-RPC

SOAP
SOAP Architecture consists of several layers
Message Format
...
gRPC
gRPC
 is a modern, open-source RPC framework designed for high-performance communication
gRPC
 is built on top of HTTP/2
...
gRPC
 supports communication between micro-services written in different languages
There is a description of the API contracts (Protocol Buffer)
Server and client use stub codes for the methods (
Protcol
 Buffer compiler and 
gRPC
 plugins)
...
GetStockQuote
 method, processes the request and returns the stock price
 
Data structures are defined in the 
.proto 
file, and the Protocol Buffer compiler will generate source code for languages like C++, Python, Go, Java and more
syntax = "proto3";
package stock;
...
StockService
 {
  // RPC method to get a stock quote for a given symbol.
...
the client to pick it apart (using SAX, DOM, XSLT, etc.) 
– more in Lecture 6/Client-side scripting
...
Requires some additional coding to handle either end
	i.e. XML Parsing
In a sense, HTTP itself is a 
ReSTful
 application, with the HTTP being interpreted by the server and user agent
...
REST Example
Assume that the resource is represented by 
...
‘https://api.spotify.com/v1/search?type=artist&q=snoop
’
try
:
	response = 
...
No 
params
 required, generates a GET request
...
url
 =“
http://api.musicfun.com/v1/playlists
”
...
print
(result)
The Request object now has some encoded data – this is a POST request
...
Real-world applications: RESTful APIs and 
gRPC
