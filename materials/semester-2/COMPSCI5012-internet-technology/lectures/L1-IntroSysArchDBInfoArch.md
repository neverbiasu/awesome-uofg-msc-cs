...
tiered architectures
 and their benefits
3. Interpret and create 
high-level architecture diagrams
4. Understand 
data modeling
 and create 
ER diagrams
5. Apply Information Architecture principles to develop 
user requirements
, 
sitemaps
, 
...
, and 
wireframes 
This lecture covers most of what you need for the first part of your group coursework!
...
Internet vs Web
Internet: 
the network connecting computers 
Web: 
what browsers use on that network – links, pages, web applications
 
...
Need for principled and disciplined approach…
Design
...
We focus on the application level or the 
high-level system architecture
...
Monolithic Programs
Example: A monolithic program in JAVA performs all the tasks
...
This reflects a 1-tier architecture – not suitable for web applications!
...
applications
 have changed from: 
Monolithic
 programs in which every aspect of the application is coded
To 
tiered
 structures in which different aspects are separated into different levels
The 
advantage
 of the 
tiered
...
architectures
 are 
Each tier can be coded 
separately
 without the programmer having to deal with everything
...
-
consuming
 to write the code which accesses large amounts of data efficiently
Solution: Separate the concerns
Let
 Database Systems
 to handle the data management
And then use a 
Client application
 to interact with the database:
The client acts like a database user sending in queries and updates and making use of the result
The standard mechanism is to code 
SQL statements
 as strings inside the program
...
The text/data (html) and the style (
css
), 
With lots of pages there is lots of overhead in creating and maintaining the site.
Solution: Separate the concerns
Use external Cascading Style Sheets (CSS) which act upon the different elements in the html.
The web server/application returns a html file and 
css
 file.
...
Layers within the client tier handles the content and look/feel.
		
Two Tier with Layers
The 
content layer 
represents/stores the data
The 
presentation layer 
renders the data
...
Web Server 
handles incoming requests and send outgoing responses, routing them to/from the correct application server
	
Application Server
 is typically a script that dynamically generates a response given a request
		

Two Tier Issues
Distributed applications with a 
two-tiered architecture
 distinguish only:
client
 processes – control the application logic and the user interface
server
 processes – supply resources, such as data, to the clients
however this:
...
An improvement is to separate the application into 
three (or N-) tiers
:
presentation
 processes deal exclusively with the user interface – this might, for instance, be through the use of a browser or other user agent
application 
processes deal with the logic of the application, queries, calculations, etc. – there is either one (3-tier) or more (N-tier) of such processes
data source
 processes supply the data from a database (binary) or a file (for instance, XML)
...
where the client handle the interaction with the user, the middleware handles the application logic, and the database stores the data.
	
Database: 
persists and manages the data associated with the application.
...
Webserver
: handles incoming responses, directing them to the appropriate server.
...
Using a 
Domain Name Server 
such that when a URL is resolved, it rotates through a series of IP addresses that route the message to that machine.
Using a 
Load Balancing Server
, which farms out the requests to available machines. 
...
high level design 
is useful because:
it helps to describe the system at a level which makes the 
goals, scope 
and
 responsibilities
 clear
the abstraction provides a tool for communicating the design
permits the specific technologies to be 
chosen late
 or to be 
changed
maintenance
...
re-usability
 also profit
accessibility
 variants can considered 
a priori

Bottom Up Design
Piecing together components to give rise to more complex systems, thus making the original elements sub-systems of the 
emergent system
...
This strategy often resembles a 
"seed" model
...
etc
An API for other systems, agents, developers, 
etc
Devices and Robots
Sensors 
...
The 
response message
 is sent from the server to a user agent to:
...
MySql
 Database Server
Chrome; Firefox; Edge
Apache Web Server; Django 4.1.5;Bootstrap 
MySQL
...
Provides an abstract representation of the data and how it relates to each other
Developed by Peter Chen in 1976
Lend themselves to being implemented in a database (esp. relational databases)
...
Martin’s
Etc
Each support different ways to draw the ER model
We will be using a 
...
vs
 Compressed Chen (2)
In compressed Chen notation, we only put entities and relationships in the diagram and then separately list the attributes
...
Then the relationship between the entities is specified using:
models.ForeignKey
  - denotes one-to-many
...
https://docs.djangoproject.com/en/1.10/topics/db/examples/
 for more details
...
art
 and 
science
 of shaping information products and experiences to support 
usability
 and 
findability.
...
- Assists to 
identify user-based requirements
.
Is responsible for 
how users interpret and interact with information. 
freeing up visual designers to concentrate on visual design elements and programmers to concentrate on code.
Investigate 
customers and their needs,
 factor 
business strategy and technology resources
 into 
solutions
. 
...
Rosenfield and 
Morville
 (the polar bear book): https://
www.oreilly.com
/library/view/information-architecture-for/0596527349/
...
The fundamental questions an IA seeks to answer is:
Who is the user?
What do they need?
What will they see?
How will they interact with the system?
How will they get value from, accomplish their goals/tasks, 
etc
?

IA Deliverables	
To answer such questions, IA have a number of tools and techniques that they employ, these include:
Specifying user requirements
Showing the structure of the site through site maps
Showing the sequence of interaction through user flows 
Mocking up Wireframes to show structure of screens/pages

Specifying User Requirements
Many ways to specify user requirements
1. Simple list of requirements from the point of the user / User stories
...
Creates a snapshot view of the user eco-system
 ...
 Personas
User archetypes to help to guide decisions about product features, navigation, interactions, and even visual design 
 Demographics (User group)
 Needs (Goals)
 Environmental (Context)
https://
www.usability.gov
/how-to-and-tools/methods/
personas.html
...
MoSCoW
 method
Must have
: essential for the primary user scenario (no workaround)
Example: sign up/log in; create/edit core item; view list/detail; input validated & saved
Should have
: very important, but a temporary workaround exists
Example: search/filter; password reset; 
userprofile
 settings
Could have
...
Site Maps
Blueprints that shows how the site is going to be organized (sections/URLs). 
Provides the high level snapshot view and relationship between pages of the site.
Helps in restructuring the deep hierarchies.
Helps to figure out navigation.
...
Use lowercase characters and avoid special characters in URLS
Use static URLS when you can
This lets users revisit the information/page
Means that crawlers can index the content

User flows
Diagram that shows how a user moves through the site accomplishing a task
https://
careerfoundry.com
/
en
...
Shows what a user has to accomplish to achieve a goal
Let’s you reduce complexity of interaction

Wireframes
Depict how an individual page or template will look from an architectural perspective. They are the intersection of the site’s IA and its visual and information design.
Saves lot of time
...
Keep the wireframe simple, abstract, but representative.
...
Wireframe with the team in mind 
– what can be successfully developed?
Use real content 
– to show 
clients
 what to expect and what they will see

Example specification wireframe

Tools for drawing wireframes
...
Accessibility
How can people with a disability access/use the web?
A very overlooked topic in web design!
A mixture of good design and coding
For example, always provide ALT text for images (even if it is empty)
https://www.w3.org/WAI/standards-guidelines/wcag/
 
Up to 20% of users have some form of disability
Standards and guidelines to make web accessible
Increasing focus on web applications rather than static web sites
...
DB Modelling
Modeling through ERDs allows you to think about the data and its relationship
