...
Construct Structured HTML Documents
Apply CSS for Styling and Layout
Understand how front-end frameworks (e.g. Bootstrap) can ease web development
Understand Django Database modeling, and using data in templates (own study)
Learn about accessibility and sustainability in web design and development
...
Lewis Coulson, Brett Jephson, Rob Larsen, Matt Park, and Marian 
Zburlea
...
Hypertext 
is the link between documents
A 
mark-up language
 is a set of tags which describe document content
...
<html>
 tag
<head>
 tag contains information about the document such as title and other things
<body>
 tag contains the html to be displayed
<html>
	<head>
	   <title> The title</title>
	</head>
...
HTML Elements
Elements
Everything from an opening tag to a closing tag is called an 
element
The plain text between opening and closing tags is called the 
element content
<html>
	<head>
	   <title>The title</title>
...
>
Div
 elements let you create sections to divide up the page in different ways when coupled with CSS.
	<div> </div>	

HTML First
HTML First: A writing style for web development -- not necessary the best
...
Data Centers:
 High energy use to store data
Networks:
  High energy use to transfer data
User devices: 
High 
emergy
 use to load/interact with data
...
24 guidelines relevant to Web Development 
(Section 3): 
...
Style on the Web
Decisions of Style: 
Most aspects about any element of a web page can be controlled:
...
separating a document’s structure and content from its presentation 
– remember 
separation of concerns
?
CSS allows for a much 
richer document appearance 
than (X)HTML alone
CSS can save time as the appearance of the entire document can be 
created and changed in just one place
CSS can improve load times as it 
compactly stores the presentation concerns
 
of a document in one place instead of being repeated throughout the document
...
they specify some style for 
individual elements 
or 
all elements 
of a particular kind
A CSS consists of a set of 
formatting rules, 
which are specified in the following way:
selector
 indicates the element (or set of)
property
 refers to the stylistic aspect
value
...
Apply to all <a> elements with id=“menu”

CSS Values and Units
Units 
affect the colors, distances, and sizes of a whole host of properties of an element’s style
Numbers
...
Color
named color (e.g. ‘red’), functional 
rgb
(0,0,255), or hexadecimal RGB codes (#FF0000) 
...
pt
 – 72pt to 1 inch), and picas (
pi
 – 1pi = 12pt)
Relative Length Units
em
 is relative to the given font-size value 
e.g. font-size is 14px, 1em = 14px
ex
 is relative to the size of a lowercase x for the given font family
px
 is (should be) the size of a pixel on the monitor
px
 is generally the recommended unit to use 
...
Inline CSS
 only affects this element, and others of the same type are not affected.  
Useful to 
override 
existing style
but
...
The CSS rules will be applied to the 
entire document
<html>
	<head>
...
<style>
	...
	<body>
		… <h3> This will be in yellow font size 18
...
It is also very important for manipulating elements in 
Javascript
The effort also pays off if you use libraries like 
JQuery
Class selectors:
 work on a set of specified elements through the 
class attribute 
ID selectors:
 provide a way to stylize unique elements  through the 
id attribute
...
They work when the 
class attribute
 of an HTML tag is given a 
name
 
The 
dot (.)
 operator is used to define the class 
<style>
...
p
>
This text will NOT be displayed in bold.
</
p
>
...
The 
hash symbol (#)
 is used to specify a unique ID
<style>
	#first-
para
 { font-weight: bold; }
...
”
>
This paragraph will be bold-faced
</p>
<
p
...
Elements that are descended from a particular element are styled according to the rule of the 
descendent selector
...
You might specify 
conflicting rules
 in your CSS e.g. different values of the same property apply to the same element
In this case it’s important to understand how these rules get sorted out through 
cascades
, 
specificity
...
For example, below <
em
> will inherit the style of its parent <
p
>
<style>
	p { color: red; 
...
When rules conflict, CSS uses 
weights to decide which one applies
...
CSS is based on a method of causing 
styles to cascade together
, which is made possible by combining inheritance, specificity and order
The purpose of “
cascading
” is to find one winning rule among a set of rules that apply to a given element
In essence, 
if two rules have the same specificity, then the one that is defined last in the stylesheet wins
!

The Box Model

The Box Model
Every element generates one or more rectangular 
element boxes 
 which houses the content. The element box is surrounded by optional amounts of 
padding
, 
borders
, and 
...
CSS floating properties allow you to 
float
...
CSS positioning properties allow you to 
position
 an element.
...
right
 properties.
There are four different ways to position: static (default), 
fixed
, 
relative
 and 
...
has appeared in the incorrect location.
...
These newer methods are also 
responsive
 so can deal with different screen sizes
...
Guideline #3.13:
 Adapt to User Preferences
	...
	- CSS media/preference queries should enable users, e.g. 
monochrome
, 
...
The median webpage is 2.3MB – 600% up in the last 10 years
Optimize media
...
What is Bootstrap?
Another web application framework (WAF) 
...
https://getbootstrap.com/
 provides different releases. Latest is 5.*, TWD book uses 4
Go to w3schools and follow the 
tutorial
 (also other good resources out there)
