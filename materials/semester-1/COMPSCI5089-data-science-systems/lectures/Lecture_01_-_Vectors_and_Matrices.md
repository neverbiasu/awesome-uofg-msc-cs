...
- R≥0 denotes the set of non-negative reals.
- R^n denotes the set of tuples of exactly n real numbers (vector).
- R^(n×m) denotes the set of 2D arrays (matrix) of real numbers arranged in n rows by m columns.
- The notation (R^n, R^n) → R says that the operation defines a map from a pair of n dimensional vectors to a real number.

**Vector spaces**
Any vector of given dimension n lies in a vector space, called R^n, along with the operations of:
- **scalar multiplication** so that a**x** is defined for any scalar a. For real vectors, a**x** = [ax₁, ax₂, ... axn], elementwise scaling.
  - (R, R^n) → R^n
- **vector addition** so that **x** + **y** vectors **x**, **y** of equal dimension. For real vectors, **x** + **y** = [x₁ + y₁, x₂ + y₂, ... xd + yd] the elementwise sum
  - (R^n, R^n) → R^n

**Two additional operations**
- **norm** || x || which allows the length of vectors to be measured.
  - R^n → R≥0
- **inner product** (<x | y>), (<x,y>) or x · y which determines the angle between two vectors. The inner product of two orthogonal vectors is 0. For real vectors
  - x · y = x₁y₁ + x₂y₂ + x₃y₃ ... xdyd
  - (R^n, R^n) → R
...
- **Topological/geometrical space**: a set whose elements are called points
- **Metric space**: define distance between points, e.g. norm
- **Normed vector space**: a vector space with norm defined

- **Relationship between Inner Product and Norm**:
  - If you have an inner product on a vector space, you can derive a norm from it: ||x|| = √(<x|x>)
...
- Each row can be seen as a vector in R^n
...
- Splitting images into patches, and treating each patch as a vector x_1,...,x_n
- The vectors are **clustered** to find a small number of vectors y_1,...,y_m, m<<n that are a reasonable approximation of nearby vectors.
...
- The L2-norm or Euclidean length of a vector x (written as ||x||) can be computed directly using `np.linalg.norm()`.
- This is equal to:
  - $$ ||x||_2 = \sqrt{x_0^2 + x_1^2 + x_2^2 + ... + x_n^2} $$

## Different norms

- **Euclidean norm** or Euclidean distance measure
  - $$ ||x - y||_2 $$
- **Lp-norms** or **Minkowski** norms, which is defined by:
  - $$ ||x||_p = (\sum_i x_i^p)^{1/p} $$

| p | Notation | Common name | Effect | Uses | Geometric view |
|---|---|---|---|---|---|
| 2 | ||x|| or ||x||₂ | Euclidean norm | Ordinary distance | Spatial distance measurement | Sphere just touching point |
| 1 | ||x||₁ | Taxicab norm; Manhattan norm | Sum of absolute values | Distances in high dimensions, or on grids | Axis-aligned steps to get to point |
| 0 | ||x||₀ | Zero pseudo-norm; non-zero sum | Count of non-zero values | Counting the number of "active elements" | Numbers of dimensions not touching axes |
| ∞ | ||x||_inf | Infinity norm; max norm | Maximum element | Capturing maximum "activation" or "excursion" | Smallest cube enclosing point |
| -∞ | ||x||_-inf | Min norm | Minimum element | Capturing minimum excursion | Distance of point to closest axis |
...
- An **inner product** (R^N × R^N) → R measures the **angle** between two real vectors.
- It is related to the cosine distance:
  - $$ \cos{\theta} = \frac{x \cdot y}{||x|| \cdot ||y||} $$
- For unit vectors, we can forget about the denominator, since || x ||= 1, || y ||= 1, so cos θ = x · y.
- The computation of the inner product, for real-valued vectors in R^N, is simply the sum of the elementwise products:
...
- **mean vector** of a collection of N vectors
  - $$ mean(x_1, x_2, ..., x_N) = \frac{1}{N} \sum_i x_i $$
...
- where each element of the matrix A is written as A_i,j, for the ith row and jth column.
...
- An n×m matrix **A** represents a function f(x) taking m dimensional vectors to n dimensional vectors (R^m → R^n)
...
- A **linear map** is any function f: R^m → R^n which satisfies the **linearity** requirements.
- If the map represented by the matrix is n x n then it maps from a vector space onto the **same** vector space (e.g. from R^n → R^n), and it is called a **linear transform**.
...
- If A is R^(n×m), and x is R^(m×1), then this will map from an m dimensional vector space to an n dimensional vector space: (R^(n×m), R^m) → R^n.
...
In particular, we take each element of x, x₁, x₂, ..., xm, multiply it with the corresponding column of A, and sum these columns together.
- Set y = [0,0,0, ...] = 0^n (the n-dimensional zero vector)
- For each column 1 ≤ i ≤ m in A
- y = y + xᵢAᵢ. Note that xᵢAᵢ is scalar times vector, and has n elements. Aᵢ here means the i th column of A.

## Matrix multiplication

- **Matrix multiplication** defines the product C=AB, where A,B,C are all matrices.
- Matrix multiplication is defined such that if **A** represents linear transform f(x) and **B** represents linear transform g(x), then **BAx**=g(f(x))
...
- Matrix multiplication has, in the general case, of time complexity O(pqr), or for multiplying two square matrices O(n³).
...
- There are some accelerated algorithms for general multiplication. The time complexity of all of them is >O(N²) but <O(N³).
...
- **outer product**: the product of a M×1 with a 1×N vector, which is an M×N matrix
  - $$ x \otimes y = x^T y $$
- **inner product**: the product of a 1×N with an N×1 vector is a 1x1 matrix, which is a scalar
  - $$ x \cdot y = xy^T $$
...
- If you have multiple variables X₁ . . . Xn
